...
export function ProjectsView() {
  ...
  return <div className="space-y-6"><PageTitle eyebrow="Project workspace" title="Projects" /><div className="grid gap-5 md:grid-cols-2 xl:grid-cols-3">{projects.map((project) => <ProjectCard key={project.id} project={project} />)}</div></div>;
}
  ...
  return (
    <div className="space-y-6"><PageTitle eyebrow={projects.find((project) => project.id === meeting.project_id)?.name} title={meeting.title} />
      <div className="grid gap-6 xl:grid-cols-[1fr_1fr_0.9fr]">
        ...
        <Card><SectionHeader title="Action items" />{actionItems.filter((item) => item.meeting_id === meeting.id).map((item) => <p key={item.id} className="mb-3 rounded-xl bg-eventum-elevated p-3 text-sm">{item.title}<br /><span className="text-eventum-muted">Owner: {users.find((user) => user.id === item.owner_id)?.name} | {item.status}</span></p>)}<SectionHeader title="AI suggestion block" eyebrow="Mock only" />{ai?.tasks?.map((task) => <p key={task.title} className="text-sm text-eventum-muted">{task.title} - {task.owner_suggestion}</p>)}{ai?.risks?.map((risk) => <p key={risk} className="mt-2 text-sm text-eventum-cinnabar">{risk}</p>)}<SectionHeader title="Linked tasks" />{tasks.filter((task) => actionItems.some((item) => item.task_id === task.id && item.meeting_id === meeting.id)).map((task) => <p key={task.id} className="text-sm text-eventum-muted">{task.title}</p>)}</Card>
      </div>
    </div>
  );
}
...
export function AiAssistantView() {
  ...
  return <div className="space-y-6"><PageTitle eyebrow="Quiet helper" title="AI Assistant" /><div className="grid gap-6 xl:grid-cols-[0.7fr_1fr_1fr]"><Card><SectionHeader title="Prompt cards" eyebrow="Mock assistant" />{aiOutputs.map((output) => <button key={output.id} onClick={() => setSelected(output)} className="mb-2 w-full rounded-lg bg-eventum-elevated px-3 py-2 text-left text-sm text-eventum-muted transition hover:bg-eventum-soft hover:text-eventum-text">{output.type.replace("_", " ")}</button>)}</Card><Card><SectionHeader title="Source text" /><textarea defaultValue={selected.input_text} className="min-h-64 w-full rounded-xl border border-eventum-border bg-eventum-elevated p-3 text-sm outline-none focus:border-eventum-borderStrong" /><p className="mt-3 text-sm text-eventum-dim">No real AI call is made in Phase 2.</p></Card><Card><SectionHeader title="Suggestion preview" eyebrow="AI draft" /><pre className="max-h-96 overflow-auto rounded-xl border border-eventum-border bg-eventum-elevated p-4 text-xs leading-5 text-eventum-muted">{JSON.stringify(selected.output_json, null, 2)}</pre><div className="mt-4 flex gap-2"><Button>Approve selected</Button><Button variant="ghost">Reject</Button></div></Card></div></div>;
}
...
export function SettingsView() {
  ...
  return <div className="space-y-6"><PageTitle eyebrow="Simple workspace settings" title="Settings" /><div className="grid gap-6 xl:grid-cols-[0.8fr_1.2fr]"><Card><SectionHeader title="Team members" />{users.map((user) => <div key={user.id} className="mb-3 rounded-xl bg-eventum-elevated p-3"><p className="font-medium">{user.name}</p><p className="text-sm text-eventum-muted">{user.operationalArea}</p></div>)}<SectionHeader title="Profile" /><p className="text-sm text-eventum-muted">{currentUser.name} - {currentUser.role}</p></Card><Card><SectionHeader title="Routine definitions" action={currentUser.role === "founder" ? <Button onClick={() => { createRoutine({ title: "Mock routine created in settings", description: "Local-only routine definition.", owner_id: "user-assistant-1", project_id: projects[0].id, frequency: "daily", priority: "normal" }); setCreated(true); }}>Mock-create routine</Button> : null} />{created ? <p className="mb-3 text-sm text-eventum-muted">Routine created locally.</p> : null}{routines.map((routine) => <div key={routine.id} className="mb-3 flex items-center justify-between rounded-xl bg-eventum-elevated p-3"><div><p className="font-medium">{routine.title}</p><p className="text-sm text-eventum-muted">{projects.find((project) => project.id === routine.project_id)?.name} | {routine.frequency}</p></div>{currentUser.role === "founder" ? <Button variant={routine.active ? "secondary" : "ghost"} onClick={() => toggleRoutineActive(routine.id)}>{routine.active ? "Active" : "Inactive"}</Button> : <Badge>{routine.active ? "Active" : "Inactive"}</Badge>}</div>)}</Card></div></div>;
}
